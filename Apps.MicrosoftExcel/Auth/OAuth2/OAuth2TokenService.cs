...
namespace Apps.MicrosoftExcel.Auth.OAuth2;
...
public class OAuth2TokenService : BaseInvocable, IOAuth2TokenService, ITokenRefreshable
{
    ...
    private const string LogPrefix = "[MicrosoftExcel][OAuth]";
    ...
    { 
        ...
        if (!values.TryGetValue(RefreshTokenKeyName, out var refreshToken) || string.IsNullOrWhiteSpace(refreshToken))
        {
            throw new PluginMisconfigurationException(
                "The Microsoft Excel connection has no refresh token stored, so it cannot be renewed automatically. " +
                "Please reconnect your Microsoft Excel connection.");
        }
